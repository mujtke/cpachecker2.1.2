  ...
  private CExpressionAssignmentStatement getAssignmentStatement(CFAEdge pEdge, Var pGAVar) {
    if(pEdge != null) {
      if(pEdge instanceof CStatementEdge) {
        ...
        Optional<AAstNode> rtStmt = ((CReturnStatementEdge) pEdge).getRawAST();
        Optional<CReturnStatement> rtStmt2 = Optional.of((CReturnStatement)rtStmt.get());
        if (rtStmt.isPresent()) {
          Optional<CAssignment> rtAsg = rtStmt2.get().asAssignment();
        ...
        if (fcActualParms != null && !fcActualParms.isEmpty()) {
          if (fcActualParms.size() == 1) {
            // yzc modified 2022.05.23
            Optional<AAstNode> fcStmt = ((CFunctionCallEdge) pEdge).getRawAST();
            Optional<CFunctionCall> fcStmt2 = Optional.of((CFunctionCall)(fcStmt.get()));
            if (fcStmt.isPresent()) {
              // get formal parameters.
              List<CParameterDeclaration> fcFormalParms =
                  fcStmt2.get().getFunctionCallExpression().getDeclaration().getParameters();
