...
import org.sosy_lab.cpachecker.cpa.lockOld.LockState.LockTreeNode;
import org.sosy_lab.cpachecker.cpa.racer.storage.UsageDelta;
...
public final class UsageInfo implements Comparable<UsageInfo> {
  ...
  public static UsageInfo createUsageInfo(
    ...
    if (ident instanceof SingleIdentifier) {
      ...
      for (CompatibleState s : AbstractStates.asIterable(state).filter(
          CompatibleState.class)) {
       ...
       * 修改使其能够正常传入
       */
      AbstractStateWithLocations childStateWithLocs = AbstractStates.extractStateByType(state, RacerThreadingState.class);
      CFANode node = null;
      ...
      for (ARGState s : ((ARGState)state).getParents()) {
        AbstractStateWithLocations parentStateWithLocs = AbstractStates.extractStateByType(s, RacerThreadingState.class);
