...
@Options(prefix = "cpa.racer.errorPrinter")
public abstract class RacerErrorTracePrinter {
    ...
    protected RacerUsageContainer container;
    ...
    protected RacerErrorTracePrinter(
            Configuration c,
        ...
        config = c;
        lockTransfer = lT;
        config.inject(this, RacerErrorTracePrinter.class);
        ...
        if (filterMissedFiles) {
            FILTER_EMPTY_FILE_LOCATIONS =
                    Predicates.and(
                            FILTER_EMPTY_FILE_LOCATIONS,
                            e -> Files.exists(Paths.get(
                                String.valueOf(e.getFileLocation().getFileName()))));
        }
    ...
    public void printErrorTraces(RacerUsageReachedSet uReached) {
    ...
    protected List<CFAEdge> getPath(UsageInfo usage) {
        ...
        if (path != null) {
            ...
            if (subgraphComputer != null) {
                ...
                // TODO here we don't use BAM, so subgraphComputer must be null
                aPath = null;
            } else {
