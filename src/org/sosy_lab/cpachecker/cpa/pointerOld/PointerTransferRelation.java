...
public class PointerTransferRelation extends SingleEdgeTransferRelation {
  ...
  public Collection<? extends AbstractState> getAbstractSuccessorsForEdge(
    ...
    PointerState
        pointerState = (PointerState) pState;
    PointerState
        resultState = getAbstractSuccessor(pointerState, pCfaEdge);
  ...
  private PointerState getAbstractSuccessor(PointerState pState, CFAEdge pCfaEdge)
    ...
    org.sosy_lab.cpachecker.cpa.pointerOld.PointerState resultState = pState;
    transferTime.start();
    try {
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleFunctionReturnEdge(PointerState pState, CFunctionReturnEdge pCfaEdge)
    ...
    if (callEdge instanceof CFunctionCallAssignmentStatement) {
      ...
      Optional<RCUMemoryLocation> returnVar =
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleAssumeEdge(PointerState pState, AssumeEdge pAssumeEdge)
      throws UnrecognizedCodeException {
  ...
  private Optional<Boolean> areEqual(
      org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pPointerState, AExpression pOperand1, AExpression pOperand2)
      throws UnrecognizedCodeException {
  ...
  private static Optional<Boolean> pointsTo(
      org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pState, ExplicitLocationSet pLocations, CExpression pCandidateTarget)
      throws UnrecognizedCodeException {
    if (pLocations.getSize() == 1) {
      ...
      if (candidateTargets instanceof ExplicitLocationSet) {
        ...
        RCUMemoryLocation location = pLocations.iterator().next();
        ...
        if (actualTargets instanceof ExplicitLocationSet && !explicitCandidateTargets.isBot()) {
          ...
          for (RCUMemoryLocation candidateTarget : explicitCandidateTargets) {
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleReturnStatementEdge(PointerState pState, CReturnStatementEdge pCfaEdge)
      throws UnrecognizedCodeException {
    if (!pCfaEdge.getExpression().isPresent()) {
      return pState;
    }
    Optional<RCUMemoryLocation> returnVariable =
  ...
  private Optional<RCUMemoryLocation> getFunctionReturnVariable(FunctionEntryNode pFunctionEntryNode) {
    Optional<? extends AVariableDeclaration> returnVariable =
        pFunctionEntryNode.getReturnVariable();
    if (!returnVariable.isPresent()) {
      return Optional.empty();
    } else {
      return Optional.of(RCUMemoryLocation.valueOf(returnVariable.get().getQualifiedName()));
    }
  }

  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleFunctionCallEdge(
      org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pState, CFunctionCallEdge pCFunctionCallEdge) throws UnrecognizedCodeException {
    org.sosy_lab.cpachecker.cpa.pointerOld.PointerState newState = pState;
    ...
    for (Pair<CParameterDeclaration, CExpression> param : Pair.zipList(formalParams, actualParams)) {
      ...
      RCUMemoryLocation location = toLocation(formalParam);
  ...
  private RCUMemoryLocation toLocation(AbstractSimpleDeclaration pDeclaration) {
    return toLocation(pDeclaration.getType(), pDeclaration.getQualifiedName());
  }

  private RCUMemoryLocation toLocation(
    ...
    return fieldReferenceToRCUMemoryLocation(pParent, false, pMemberDecl.getName());
  }

  private RCUMemoryLocation toLocation(Type pType, String name) {
    Type type = pType;
    if (type instanceof CType) {
      type = ((CType) type).getCanonicalType();
    }
    if (isStructOrUnion(type)) {
      return RCUMemoryLocation.valueOf(type.toString()); // TODO find a better way to handle this
    }
    return RCUMemoryLocation.valueOf(name);
  }
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleStatementEdge(PointerState pState, CStatementEdge pCfaEdge)
      throws UnrecognizedCodeException {
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleAssignment(
      org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pState,
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleAssignment(
    ...
    if (useFakeLocs && pRightHandSide.isBot() && locations instanceof ExplicitLocationSet) {
      for (RCUMemoryLocation loc : ((ExplicitLocationSet) locations)) {
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleAssignment(
    ...
    final Iterable<RCUMemoryLocation> locations;
    ...
    org.sosy_lab.cpachecker.cpa.pointerOld.PointerState result = pState;
    for (RCUMemoryLocation location : locations) {
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleAssignment(
      org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pState, RCUMemoryLocation pLhsLocation, CRightHandSide pRightHandSide)
      throws UnrecognizedCodeException {
    return handleAssignment(pState, pLhsLocation, asLocations(pRightHandSide, pState, 1));
  }

  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleAssignment(PointerState pState, RCUMemoryLocation pLeftHandSide, LocationSet pRightHandSide) {
    return pState.addPointsToInformation(pLeftHandSide, pRightHandSide);
  }

  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleDeclarationEdge(
      final org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pState, final CDeclarationEdge pCfaEdge) throws UnrecognizedCodeException {
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleDeclaration(PointerState pState, CVariableDeclaration pDeclaration)
      throws UnrecognizedCodeException {
    CInitializer initializer = pDeclaration.getInitializer();
    RCUMemoryLocation location = toLocation(pDeclaration);
  ...
  private org.sosy_lab.cpachecker.cpa.pointerOld.PointerState handleWithInitializer(
      org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pState,
      RCUMemoryLocation pLeftHandSide,
      CType pType,
    ...
    if (pInitializer instanceof CInitializerList
      ...
      if (compositeType.getKind() == ComplexTypeKind.STRUCT) {
        ...
        org.sosy_lab.cpachecker.cpa.pointerOld.PointerState current = pState;
  ...
  private static LocationSet toLocationSet(Iterable<? extends RCUMemoryLocation> pLocations) {
    if (pLocations == null) {
      return LocationSetTop.INSTANCE;
    }
    Iterator<? extends RCUMemoryLocation> locationIterator = pLocations.iterator();
  ...
  public static RCUMemoryLocation fieldReferenceToRCUMemoryLocation(CFieldReference pFieldReference) {
    return fieldReferenceToRCUMemoryLocation(
        pFieldReference.getFieldOwner().getExpressionType(),
        pFieldReference.isPointerDereference(),
        pFieldReference.getFieldName());
  }

  private static RCUMemoryLocation fieldReferenceToRCUMemoryLocation(
    ...
    return RCUMemoryLocation.valueOf(prefix + infix + suffix);
  }

  private static LocationSet asLocations(
      final CRightHandSide pExpression, final org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pState, final int pDerefCounter)
      throws UnrecognizedCodeException {
    return pExpression.accept(
        new CRightHandSideVisitor<LocationSet, UnrecognizedCodeException>() {
          ...
          public LocationSet visit(CArraySubscriptExpression pIastArraySubscriptExpression)
            ...
            if (pIastArraySubscriptExpression.getSubscriptExpression()
              ...
              if (literal instanceof CIntegerLiteralExpression
                ...
                Set<RCUMemoryLocation> result = new HashSet<>();
                for (RCUMemoryLocation location : ((ExplicitLocationSet) starredLocations)) {
          ...
          public LocationSet visit(final CFieldReference pIastFieldReference)
              throws UnrecognizedCodeException {
            RCUMemoryLocation memoryLocation =
                fieldReferenceToRCUMemoryLocation(pIastFieldReference);
          ...
          public LocationSet visit(CIdExpression pIastIdExpression)
            ...
            final RCUMemoryLocation location;
            if (isStructOrUnion(type)) {
              location =
                  RCUMemoryLocation.valueOf(type.toString()); // TODO find a better way to handle this
            } else {
              CSimpleDeclaration declaration = pIastIdExpression.getDeclaration();
              if (declaration != null) {
                location = RCUMemoryLocation.valueOf(declaration.getQualifiedName());
              } else {
                location = RCUMemoryLocation.valueOf(pIastIdExpression.getName());
              }
            }
            return visit(location);
          }

          private LocationSet visit(RCUMemoryLocation pLocation) {
            Collection<RCUMemoryLocation> result = Collections.singleton(pLocation);
            for (int deref = pDerefCounter; deref > 0 && !result.isEmpty(); --deref) {
              Collection<RCUMemoryLocation> newResult = new HashSet<>();
              for (RCUMemoryLocation location : result) {
          ...
          public LocationSet visit(CFunctionCallExpression pIastFunctionCallExpression)
            ...
            return visit(RCUMemoryLocation.valueOf(declaration.getQualifiedName()));
          }
   ...
   */
  public static LocationSet asLocations(CExpression pExpression, final org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pState)
      throws UnrecognizedCodeException {
   ...
   */
  public static Iterable<RCUMemoryLocation> toNormalSet(org.sosy_lab.cpachecker.cpa.pointerOld.PointerState pState, LocationSet pLocationSet) {
          ...
          if (derefNameExpr instanceof CFieldReference) {
            ...
            if (callstackState.isPresent()) {
              return strengthenFieldReference(
                  (org.sosy_lab.cpachecker.cpa.pointerOld.PointerState) pState, callstackState.orElseThrow(), fieldReference);
            }
          }
        }
      }
  ...
  private static Collection<? extends AbstractState> strengthenFieldReference(
    ...
    RCUMemoryLocation memoryLocation =
        PointerTransferRelation.fieldReferenceToRCUMemoryLocation(pFieldReference);
    ...
    if (targets instanceof ExplicitLocationSet) {
      ...
      for (RCUMemoryLocation target : explicitTargets) {
