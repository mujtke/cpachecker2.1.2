...
public class ExplicitLocationSet implements LocationSet, Iterable<RCUMemoryLocation> {

  private final Set<RCUMemoryLocation> explicitSet;

  private ExplicitLocationSet(ImmutableSet<RCUMemoryLocation> pLocations) {
    assert pLocations.size() >= 1;
    this.explicitSet = pLocations;
  }

  @Override
  public boolean mayPointTo(RCUMemoryLocation pLocation) {
    return this.explicitSet.contains(pLocation);
  }

  @Override
  public LocationSet addElement(RCUMemoryLocation pLocation) {
    if (explicitSet.contains(pLocation)) {
      return this;
    }
    ImmutableSet.Builder<RCUMemoryLocation> builder = ImmutableSet.builder();
    builder.addAll(explicitSet).add(pLocation);
    return new ExplicitLocationSet(builder.build());
  }

  @Override
  public LocationSet addElements(Iterable<RCUMemoryLocation> pLocations) {
    ImmutableSet.Builder<RCUMemoryLocation> builder = null;
    for (RCUMemoryLocation target : pLocations) {
  ...
  @Override
  public LocationSet removeElement(RCUMemoryLocation pLocation) {
    if (!explicitSet.contains(pLocation)) {
      return this;
    }
    if (getSize() == 1) {
    ...
    ImmutableSet.Builder<RCUMemoryLocation> builder = ImmutableSet.builder();
    for (RCUMemoryLocation location : this.explicitSet) {
  ...
  public static LocationSet from(RCUMemoryLocation pLocation) {
    return new ExplicitLocationSet(ImmutableSet.of(pLocation));
  }

  public static LocationSet from(Iterable<? extends RCUMemoryLocation> pLocations) {
    Iterator<? extends RCUMemoryLocation> elementIterator = pLocations.iterator();
    if (!elementIterator.hasNext()) {
      return LocationSetBot.INSTANCE;
    }
    ImmutableSet.Builder<RCUMemoryLocation> builder = ImmutableSet.builder();
    while (elementIterator.hasNext()) {
      RCUMemoryLocation location = elementIterator.next();
      builder.add(location);
    }
    return new ExplicitLocationSet(builder.build());
  }
  ...
  public LocationSet addElements(LocationSet pElements) {
    ...
    return pElements.addElements((Iterable<RCUMemoryLocation>) this);
  }
  ...
  @Override
  public Iterator<RCUMemoryLocation> iterator() {
