...
public class PointerState implements AbstractState {
   ...
   */
  private PersistentSortedMap<RCUMemoryLocation, LocationSet> pointsToMap;

  // Lazy initialization
  private ImmutableSet<RCUMemoryLocation> knownLocations = null;

  /**
   * Creates a new pointer state with an empty initial points-to map.
    */
  private PointerState() {
    pointsToMap = PathCopyingPersistentTreeMap.<RCUMemoryLocation, LocationSet>of();
  }
   ...
   */
  private PointerState(PersistentSortedMap<RCUMemoryLocation, LocationSet> pPointsToMap) {
   ...
   */
  public PointerState addPointsToInformation(RCUMemoryLocation pSource, RCUMemoryLocation pTarget) {
   ...
   * @return the pointer state.
   */
  public PointerState addPointsToInformation(RCUMemoryLocation pSource, Iterable<RCUMemoryLocation> pTargets) {
    LocationSet previousPointsToSet = getPointsToSet(pSource);
    if (previousPointsToSet == LocationSetTop.INSTANCE) {
      return this;
    }
   ...
   * @return the pointer state.
   */
  public PointerState addPointsToInformation(RCUMemoryLocation pSource, LocationSet pTargets) {
    if (pTargets.isBot()) {
      return this;
    }
    LocationSet previousPointsToSet = getPointsToSet(pSource);
   ...
   */
  public LocationSet getPointsToSet(RCUMemoryLocation pSource) {
  ...
  public Boolean pointsTo(RCUMemoryLocation pSource, RCUMemoryLocation pTarget) {
   ...
   * not to point to it.
   */
  public boolean definitelyPointsTo(RCUMemoryLocation pSource, RCUMemoryLocation pTarget) {
   ...
   */
  public boolean definitelyNotPointsTo(RCUMemoryLocation pSource, RCUMemoryLocation pTarget) {
   ...
   * not to point to it.
   */
  public boolean mayPointTo(RCUMemoryLocation pSource, RCUMemoryLocation pTarget) {
    return !Boolean.FALSE.equals(pointsTo(pSource, pTarget));
  }
   ...
   */
  public Set<RCUMemoryLocation> getKnownLocations() {
    ...
    knownLocations =
        ...
        Iterables.concat(
            ...
            FluentIterable.from(pointsToMap.values())
                .transformAndConcat(
                    new Function<LocationSet, Iterable<? extends RCUMemoryLocation>>() {

      @Override
      public Iterable<? extends RCUMemoryLocation> apply(LocationSet pArg0) {
  ...
  public boolean isKnownLocation(RCUMemoryLocation pLoc) {
   ...
   */
  public Map<RCUMemoryLocation, LocationSet> getPointsToMap() {
  ...
  public PointerState forget(RCUMemoryLocation pPtr) {
    return new PointerState(pointsToMap.removeAndCopy(pPtr));
  }

  public Set<RCUMemoryLocation> getTrackedRCUMemoryLocations() {
    return pointsToMap.keySet();
  }

  public static boolean isFictionalPointer(RCUMemoryLocation ptr) {
