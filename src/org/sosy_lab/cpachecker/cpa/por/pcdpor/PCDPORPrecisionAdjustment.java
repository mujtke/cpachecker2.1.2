...
public class PCDPORPrecisionAdjustment implements PrecisionAdjustment {
  ...
  private AbstractICComputer icComputer;
  private final PCDPORStatistics statistics;
  private boolean isSetUp = false;
  private final String depComputationStateType;
  ...
  private void setUp() {
    if(!isSetUp) {
      ...
      if (depComputationStateType.equals("BDD")) {
        BDDCPA bddCpa = CPAs.retrieveCPA(cpa.get(), BDDCPA.class);
        NamedRegionManager manager = bddCpa.getManager();

        try {
          icComputer = new BDDICComputer(cfa, new PredicateManager(config, manager, cfa), statistics);
        } catch (InvalidConfigurationException pE) {
          pE.printStackTrace();
        }
      } else if (depComputationStateType.equals("PREDICATE")) {
        PredicateCPA predCpa = CPAs.retrieveCPA(cpa.get(), PredicateCPA.class);
        icComputer = new PredicateICComputer(config, logManager, shutdownNotifier, predCpa, statistics);
      } else {
        icComputer = null;
      }
      isSetUp = true;
  ...
  public Optional<PrecisionAdjustmentResult> prec(
    ...
    if (pFullState instanceof ARGState) {
      ...
      PredicateAbstractState parComputeState1 =
