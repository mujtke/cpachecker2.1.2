...
public class RCUTransfer extends SingleEdgeTransferRelation{
  ...
  private final Set<RCUMemoryLocation> rcuPointers;
  ...
  private Set<RCUMemoryLocation> parseFile(Path pInput) {
    Set<RCUMemoryLocation> result = new TreeSet<>();
    try (BufferedReader reader = Files.newBufferedReader(pInput, Charset.defaultCharset())) {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (matcher.find()) {
          ...
          result.add(RCUMemoryLocation.valueOf(func, id));
        } else {
          // global identifier
          result.add(RCUMemoryLocation.valueOf(line));
        }
      }
  ...
  private RCUState assignFunctionArguments(
    ...
    RacerIdentifierCreator callerCreator = new RacerIdentifierCreator(callerName);
    RacerIdentifierCreator innerCreator = new RacerIdentifierCreator(functionName);
  ...
  private RCUState handleAssignment(CExpression left, CExpression right,
    ...
    RacerIdentifierCreator localIc = new RacerIdentifierCreator(functionName);
  ...
  private RCUState handleAssignment(
    ...
    RCUMemoryLocation rcuLoc = LocationIdentifierConverter.toLocation(left);
    RCUMemoryLocation loc = LocationIdentifierConverter.toLocation(right);
  ...
  private RCUState handleDeclaration(CDeclaration pDeclaration,
    ...
    if (pDeclaration != null && pDeclaration instanceof CVariableDeclaration) {
      RacerIdentifierCreator localIc = new RacerIdentifierCreator(pFunctionName);
