...
import org.sosy_lab.cpachecker.cpa.pointerOld.PointerState;
import org.sosy_lab.cpachecker.cpa.pointerOld.PointerTransferRelation;
...
public class RCUSearchTransfer extends SingleEdgeTransferRelation {
  ...
  public Collection<? extends AbstractState> getAbstractSuccessorsForEdge(
    ...
    Set<RCUMemoryLocation> oldPointers = oldRcuSearchState.getRcuPointers();
    Set<RCUMemoryLocation> newPointers = new TreeSet<>();
  ...
  private Set<RCUMemoryLocation> handleFunctionCallExpression(
      String pFunctionName,
      RCUSearchState state,
      CFunctionCallExpression pFc) {
    CFunctionDeclaration fd = pFc.getDeclaration();
    Set<RCUMemoryLocation> pRcuPointers = new TreeSet<>();
    if (fd != null) {
      ...
      if(fd.getName().contains(assign)) {
        ...
        addRCUMemoryLocation(pFunctionName, pointerState, pRcuPointers, params.get(0));
        addRCUMemoryLocation(pFunctionName, pointerState, pRcuPointers, params.get(1));
      }
    }
  ...
  private void addRCUMemoryLocation(String pFunctionName,
                                 PointerState pointerState,
                                 Set<RCUMemoryLocation> pRcuPointers,
                                 CExpression expression) {
    RacerIdentifierCreator identifierCreator = new RacerIdentifierCreator(pFunctionName);
    AbstractIdentifier id = expression.accept(identifierCreator);
    RCUMemoryLocation location = LocationIdentifierConverter.toLocation(id);
    ...
    if (pointerState.isKnownLocation(location)) {
      ...
      RCUMemoryLocation loc = LocationIdentifierConverter.toLocation(id);
      if (loc != null) {
  ...
  private Set<RCUMemoryLocation> handleStatementEdge(CStatementEdge pEdge,
                                          String pFunctionName,
      RCUSearchState state) {
  ...
  private Set<RCUMemoryLocation> handleFunctionCallAssignmentStatement(
      String pFunctionName,
      RCUSearchState state,
    ...
    if (leftHandSide.getExpressionType() instanceof CPointerType) {
      ...
      if (decl != null && decl.getName().contains(deref)) {
        ...
        addRCUMemoryLocation(pFunctionName, pointerState, pRcuPointers, rcuPtr);

        addRCUMemoryLocation(pFunctionName, pointerState, pRcuPointers, leftHandSide);
      }
    }
    return pRcuPointers;
  }
