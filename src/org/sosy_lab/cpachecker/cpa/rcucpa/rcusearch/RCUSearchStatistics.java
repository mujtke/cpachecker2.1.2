...
public class RCUSearchStatistics implements Statistics {
  ...
  public void printStatistics(
    ...
    Set<RCUMemoryLocation> rcuAndAliases = new TreeSet<>();
    ...
    if (output != null) {
      ...
      try (Writer writer = Files.newBufferedWriter(output, Charset.defaultCharset())) {
        for (RCUMemoryLocation loc : rcuAndAliases) {
  ...
  private Collection<RCUMemoryLocation>
      getAliases(PointerState pState, Set<RCUMemoryLocation> targetPointers) {

    rcuPointers.setNextValue(targetPointers.size());
    Collection<RCUMemoryLocation> result = new TreeSet<>();
    Set<RCUMemoryLocation> overallLocations = pState.getTrackedRCUMemoryLocations();

    for (RCUMemoryLocation pointer : targetPointers) {
      ...
      if (pointerPointTo == LocationSetTop.INSTANCE) {
        ...
        for (RCUMemoryLocation rcuLoc : (ExplicitLocationSet) pointerPointTo) {
          for (RCUMemoryLocation other : overallLocations) {
  ...
  private int getFictionalPointersNumber(Set<RCUMemoryLocation> ptrs) {
    int result = 0;
    for (RCUMemoryLocation iter : ptrs) {
